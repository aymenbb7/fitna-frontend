...
const Layout = () => {
  ...
  const { settings, siteSettings } = React.useContext(SettingsContext);
  ...
  const contactWhatsapp = siteSettings?.social_whatsapp || settings?.contact?.whatsapp || `+${DEFAULT_WA}`;
  const contactEmail    = siteSettings?.contact_email   || settings?.contact?.email    || 'info@fitna.dz';
  const contactAddress  = siteSettings?.contact_address || settings?.contact?.address  || 'الجزائر العاصمة';

  // Social URLs from API settings with safe fallbacks
  const socialInstagram = siteSettings?.social_instagram || '#';
  const socialFacebook  = siteSettings?.social_facebook  || '#';
  const socialTiktok    = siteSettings?.social_tiktok    || '#';
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
            ...
            <div>
              ...
              <div className="flex gap-4">
                <a href={socialInstagram} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:text-[#E1306C] hover:bg-white/20 transition-colors">
                ...
                <a href={socialFacebook} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:text-[#1877F2] hover:bg-white/20 transition-colors">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                  </svg>
                </a>
                <a href={socialTiktok} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:text-[#00f2fe] hover:bg-white/20 transition-colors">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
